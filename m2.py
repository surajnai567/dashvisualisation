...
def update_zone(value, value2, v3, v4):
    try:
        if value is None and value2 is None and not v4:
...
def update_language_country(radio, zone, time):

    if time:
        return [False, False,{},None,None]
    if radio == "c":
        country = get_dict_for_drop_down(get_all_country(zone))
        return [False, True, country, {}, None, None]

    if radio == "l":
        lan = get_dict_for_drop_down(get_all_lang(zone))
        print(lan)
        return [True, False, {}, lan, None,None]
...
def update_faculty(value2, value3, value4):
    try:
        return get_dict_for_drop_down\
            (get_teacher_by_student_name(value2, value3, value4))
    except Exception as e:
        return {}
...
def update_student(value1, value2, value3):
    try:
        df = get_all_student_name(value2, value3, value1)
        # plot graph
...
def update_graph(value1, value2, value3, v4, l, c):
    if v4 == 'x':
        return []

    try:
        if not l and value2:
            all_df = get_all_student_name(zone=value2, language=value1)[1]
        ...
        if not c and value2:
            all_df = get_all_student_name(country=value3,zone=value2)[1]
            all_df = get_filter_data(all_df, v4)
            details = StudentDF(all_df)
